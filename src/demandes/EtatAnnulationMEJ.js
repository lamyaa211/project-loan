...
const EtatAnnulationMEJ = () => {
  ...
  const [selectedEtatAnnulationMEJ, setSelectedEtatAnnulationMEJ] = useState(null);
  const [etatAnnulationMEJs, setEtatAnnulationMEJs] = useState([]);
  ...
  useEffect(() => {
    loadEtatAnnulationMej();
  }, []);

  const loadEtatAnnulationMej = async () => {
    try {
      const response = await axios.get('http://localhost:8084/annexes/getAllEtatAnnulationMej');
      const formattedData = response.data.map(etatAnnulationMEJ => ({
        ...etatAnnulationMEJ,
      }));
      setEtatAnnulationMEJs(formattedData);
    } catch (error) {
  ...
  const closeModal = () => {
    ...
    setIdCredit('');
    setCodeMotif('');
    setSelectedEtatAnnulationMEJ(null);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();

    const etatAnnulationMejDTO = {
      idCredit,
      codeMotif,
    };

    try {
      if (selectedEtatAnnulationMEJ) {
        await axios.put(`http://localhost:8084/annexes/updateEtatAnnulationMej/${selectedEtatAnnulationMEJ.id}`, etatAnnulationMejDTO);
      } else {
        await axios.post('http://localhost:8084/annexes/addEtatAnnulationMej', etatAnnulationMejDTO);
      }
      await loadEtatAnnulationMej();
      closeModal();
    } catch (error) {
  ...
  const handleEditClick = (etatAnnulationMEJ) => {
    setSelectedEtatAnnulationMEJ(etatAnnulationMEJ);
    setIdCredit(etatAnnulationMEJ.idCredit);
    setCodeMotif(etatAnnulationMEJ.codeMotif);
    openModal();
  };
  ...
  const handleSwitchChange = async (id, isActive) => {
    try {
      if (isActive) {
        await axios.put(`http://localhost:8084/annexes/${id}/deactivateEtatAnnulationMej`);
      }
      setEtatAnnulationMEJs(prevState => prevState.map(item => 
        item.id === id ? { ...item, isActive: !isActive } : item
      ));
      setSuccessMessage('Etat Annulation Mej  mis à jour avec succès.');
    } catch (error) {
      console.error("Erreur lors de la mise à jour de Etat Annulation Mej  :", error);
  ...
  const handleRunJobClick = async () => {
    ...
    setSuccessMessage('');
    try {
      const response = await axios.get('http://localhost:8084/batch/generateEtatAnnulationMejReport', {
        responseType: 'text',
      });
  ...
  const filteredEtatAnnulationMej = etatAnnulationMEJs.filter(etatAnnulationMEJ =>
    etatAnnulationMEJ.idCredit.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const enumOptions = [
    { code: 1, label: 'Apurement' },
    { code: 2, label: ' Rééchelonnement / Consolidation' },
  ];
    ...
    Demandes de Crédits
          ...
          <li aria-current="page">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Etat Annulation MEJ  </span>
            </div>
          </li>
        </ol>
      </nav>
...
> Ajouter un État d'Annulation MEJ
</button>
}
<button
    ...
    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr className="sticky top-0 border-b border-gray-200 bg-gray-50 dark:bg-gray-700">
                ...
                <th scope="col" className="px-3 py-2">  Code Motif </th>
            ...
            <tbody>
                {filteredEtatAnnulationMej.map((etatAnnulationMEJ, index) => (
                  <tr
                    key={index}
                  ...
                  >
                  <td className="px-3 py-2">{etatAnnulationMEJ.idCredit}</td>
                  <td className="px-3 py-2">{etatAnnulationMEJ.codeMotif}</td>
                  <td className="px-3 py-2"> 
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={etatAnnulationMEJ.isActive}
                        onChange={() => handleSwitchChange(etatAnnulationMEJ.id, etatAnnulationMEJ.isActive)}
                        className="sr-only"
                      />
                      <div className={`w-8 h-4 flex items-center rounded-full p-1 cursor-pointer ${etatAnnulationMEJ.isActive ? 'bg-[#FF8C00]' : 'bg-gray-200'}`}>
                        <div className={`w-3 h-3 bg-white rounded-full shadow-transform transition-transform ${etatAnnulationMEJ.isActive ? 'translate-x-4' : 'translate-x-0'}`}></div>
                      </div>
                      <span className="ml-2 text-gray-500 text-xs">{etatAnnulationMEJ.isActive ? 'Actif' : 'Inactif'}</span>
                    </label>
                  </td>

                  <td className="px-3 py-2">
                    <button
                      onClick={() => handleEditClick(etatAnnulationMEJ)}
                      disabled={!etatAnnulationMEJ.isActive}
                      className={`text-[#FF9119] hover:text-[#FF9119]/80 ${!etatAnnulationMEJ.isActive ? 'text-gray-400 cursor-not-allowed' : ''}`}
                    >
                      <FaEdit />
                    </button>
                  </td>
        ...
        {showModal && (
             ...
             onClick={handleBackgroundClick} >  
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
            <label htmlFor="codeMotif" className="block text-sm font-medium text-gray-700">
              Code Motif 
            </label>
            <select
              id="codeMotif"
          ...
          > {selectedEtatAnnulationMEJ ? 'Modifier' : 'Ajouter'}
          </button>
               </div>
