...
const Entreprise = () => {
  ...
  const handleSwitchChange = async (id, isActive) => {
    try {
      if (isActive) {
        await axios.put(`http://localhost:8084/annexes/${id}/deactivateProduitEntreprise`);
      }
      setProduits(prevState => prevState.map(item => 
        item.id === id ? { ...item, isActive: !isActive } : item
      ));
      setSuccessMessage('Produit Entreprise mis à jour avec succès.');
    } catch (error) {
      console.error("Erreur lors de la mise à jour de Produit Entreprise :", error);
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          ...
          <li aria-current="page">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Produits pour Entreprise</span>
            </div>
          </li>
        </ol>
      </nav>
...
<div className="overflow-y-auto max-h-80 mt-5 shadow-md rounded-lg">
    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        ...
        <th className="px-3 py-2">CODE PRODUIT</th>
            ...
            <tbody className="bg-white">
              {filteredProduits.map((produit) => (
                <tr key={produit.id} className="border-b border-gray-300">
                  ...
                  <td className="px-3 py-2">{produit.nom}</td>
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-xl w-full relative">
            ...
            <div className="mb-4">
                  ...
                  <input
                    ...
                    disabled={!!selectedProduit} // Disable input if editing a product
